...
describe('Election', () => {
  ...
  it("initializes the given candidates with the correct values", async ()=> {
    candidateInfo = await election.methods.candidates(1).call();
      ...
      assert.equal(candidateInfo.name, "Amar Singh Rai", "contains the correct name");
      assert.equal(candidateInfo.party, "All India Trinamool Congress", "contains the correct party");
      assert.equal(candidateInfo.voteCount, 0, "contains the correct votes count");
    ...
    candidateInfo = await election.methods.candidates(6).call();
      ...
      assert.equal(candidateInfo.party, "None of the above", "contains the correct party");
      assert.equal(candidateInfo.voteCount, 0, "contains the correct votes count");
  });
  ...
  it("allows a voter to cast a vote", async () => {
    ...
    assert.equal(candidateInfo.voteCount, 1, "vote count successfully increased")
  });
